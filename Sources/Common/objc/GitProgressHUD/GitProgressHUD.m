...
#define GitMainThreadAssert() NSAssert([NSThread isMainThread], @"GitProgressHUD needs to be accessed on the main thread.");

CGFloat const GitProgressMaxOffset = 1000000.f;

static const CGFloat GitDefaultPadding = 4.f;
static const CGFloat GitDefaultLabelFontSize = 16.f;
static const CGFloat GitDefaultDetailsLabelFontSize = 12.f;


@interface GitProgressHUD ()
...
@interface GitProgressHUDRoundedButton : UIButton
@end


@implementation GitProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    GitProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    GitProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (GitProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            GitProgressHUD *hud = (GitProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = GitProgressHUDAnimationFade;
    _mode = GitProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(GitProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == GitProgressHUDAnimationZoom) {
        type = animatingIn ? GitProgressHUDAnimationZoomIn : GitProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == GitProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == GitProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == GitProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == GitProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    GitProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<GitProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    GitBackgroundView *backgroundView = [[GitBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = GitProgressHUDBackgroundStyleSolidColor;
    ...
    GitBackgroundView *bezelView = [GitBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:GitDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:GitDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [GitProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:GitDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[GitRoundProgressView class]];

    GitProgressHUDMode mode = self.mode;
    if (mode == GitProgressHUDModeIndeterminate) {
    ...
    else if (mode == GitProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[GitBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == GitProgressHUDModeDeterminate || mode == GitProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[GitRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == GitProgressHUDModeAnnularDeterminate) {
            [(GitRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == GitProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == GitProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[GitProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[GitProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[GitRoundProgressView class]]) {
        GitRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [GitRoundProgressView appearanceWhenContainedIn:[GitProgressHUD class], nil];
#else
        appearance = [GitRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[GitProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((GitRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((GitRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[GitBarProgressView class]]) {
        GitBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [GitBarProgressView appearanceWhenContainedIn:[GitProgressHUD class], nil];
#else
        appearance = [GitBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[GitProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((GitBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((GitBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    GitBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? GitDefaultPadding : 0.f;
...
- (void)setMode:(GitProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == GitProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation GitRoundProgressView
...
@implementation GitBarProgressView
...
@implementation GitBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = GitProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = GitProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(GitProgressHUDBackgroundStyle)style {
    if (style == GitProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = GitProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    GitProgressHUDBackgroundStyle style = self.style;
    if (style == GitProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == GitProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation GitProgressHUDRoundedButton
